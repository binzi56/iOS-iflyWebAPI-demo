...
typedef NS_ENUM(NSInteger, HYZombieDetectStrategy) {
    HYZombieDetectStrategyCustomObjectOnly = 0, //只监控自定义对象, 默认使用该策略
    HYZombieDetectStrategyBlacklist = 1, //使用黑名单
    HYZombieDetectStrategyWhitelist = 2, //使用白名单
    HYZombieDetectStrategyAll = 3, //监控所有对象，强制过滤类除外
};
 ...
 */
@interface HYZombieDetector : NSObject
...
@property (nonatomic, assign) HYZombieDetectStrategy detectStrategy; //监控策略，默认HYZombieDetectStrategyCustomObjectOnly
@property (nonatomic, copy) NSArray<NSString*> *blackList; //黑名单，HYZombieDetectStrategyBlacklist时生效
@property (nonatomic, copy) NSArray<NSString*> *whiteList; //白名单，HYZombieDetectStrategyWhitelist生效
