...
//上报网络连接状态
extern void (*ReportLongLinkError)(ErrCmdType _err_type, int _err_code, const std::string& _ip, uint16_t _port, uint64_t last_noop_rtt,uint64_t recv_elapsetime);

//上报长连接的noop rtt
extern void (*ReportLongLinkNoopRtt)(uint64_t rtt);

//发起心跳包后再下一次心跳发起时上次心跳还没回来的次数
extern void (*ReportLongLinkNoopMiss)(bool miss);
