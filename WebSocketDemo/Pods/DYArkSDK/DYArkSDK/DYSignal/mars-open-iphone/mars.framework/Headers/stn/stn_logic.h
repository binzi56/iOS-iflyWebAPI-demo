...
namespace stn{
    ...
    public:
        ...
        //上报长连接的错误
        virtual void ReportLongLinkError(ErrCmdType _err_type, int _err_code, const std::string& _ip, uint16_t _port , uint64_t last_noop_rtt,uint64_t recv_elapsetime) = 0;
        
        //上报长连接的rtt值
        virtual void ReportLongLinkNoopRtt(uint64_t rtt) = 0;
        
        //发起心跳包后再下一次心跳发起时上次心跳还没回来的次数
        virtual void ReportLongLinkNoopMiss(bool miss) = 0;
