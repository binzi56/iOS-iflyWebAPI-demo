                        ...
                        if (valueArr) {
                            ...
                            for (id one in valueArr) {
                                if ([one isKindOfClass:meta->_genericCls]) {
                                    ...
                                    if ([newOne isKindOfClass:[GPBMessage class]]) {
                                        [GPBMessage json2pbInternal:(GPBMessage*)newOne dict:one];
                                    } else {
                        ...
                        if (meta->_genericCls) {
                            ...
                            [((NSDictionary *)value) enumerateKeysAndObjectsUsingBlock:^(NSString *oneKey, id oneValue, BOOL *stop) {
                                if ([oneValue isKindOfClass:[NSDictionary class]]) {
                                    ...
                                    if ([newOne isKindOfClass:[GPBMessage class]]) {
                                        [GPBMessage json2pbInternal:(GPBMessage*)newOne dict:oneValue];
                                    } else {
            ...
            case YYEncodingTypeObject: {
                ...
                if (isNull) {
                    ...
                    if (one) {
                        ...
                        if ([one isKindOfClass:[GPBMessage class]]) {
                            [GPBMessage json2pbInternal:(GPBMessage*)one dict:value];
                        } else {
...
static id ModelToJSONObjectRecursive(NSObject *model) {
    ...
    if ([model isKindOfClass:[GPBMessage class]]) {
        return [(GPBMessage*)model toJsonDict];
    }
...
- (id)yy_copyWithZone:(NSZone *)zone
{
   id one = [[[self class] allocWithZone:zone] init];
   _YYModelMeta *modelMeta = [_YYModelMeta metaWithClass:self.class];
    [self yy_copy2Object:one modelDeta:modelMeta mutableCopy:NO deep:NO];
    return one;
}

- (id)yy_deepCopyWithZone:(NSZone *)zone
{
    id one = [[[self class] allocWithZone:zone] init];
    _YYModelMeta *modelMeta = [_YYModelMeta metaWithClass:self.class];
    [self yy_copy2Object:one modelDeta:modelMeta mutableCopy:NO deep:YES];
    return one;
}

- (id)yy_deepMutableCopyWithZone:(NSZone *)zone
{
    id one = [[[self class] allocWithZone:zone] init];
    _YYModelMeta *modelMeta = [_YYModelMeta metaWithClass:self.class];
    [self yy_copy2Object:one modelDeta:modelMeta mutableCopy:YES deep:YES];
    return one;
}
...
- (void)yy_copy2Object:(NSObject*)one
             modelDeta:(_YYModelMeta*)modelMeta
            ...
            switch (propertyMeta->_type & YYEncodingTypeMask) {
                ...
                case YYEncodingTypeBlock: {
                    ...
                    if (deep) {
                        ...
                        if (mutableCopy && [value respondsToSelector:@selector(mutableCopyWithZone:)]) {
                            ((void (*)(id, SEL, id))(void *) objc_msgSend)((id)one, propertyMeta->_setter, [value mutableCopy]);
                        } else if ([value respondsToSelector:@selector(copyWithZone:)]){
                            ((void (*)(id, SEL, id))(void *) objc_msgSend)((id)one, propertyMeta->_setter, [value copy]);
                        } else {
                            NSAssert(NO, @"deep copy should implement copy protocol");
...
+ (instancetype)copyFromGPBMessage:(id)model{
    id one = [[self alloc]init];
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (propertyValue && [one respondsToSelector:getSel]) {
            [one setValue:propertyValue forKey:propertyName];
        }
    }
    free(properties);
    return one;
