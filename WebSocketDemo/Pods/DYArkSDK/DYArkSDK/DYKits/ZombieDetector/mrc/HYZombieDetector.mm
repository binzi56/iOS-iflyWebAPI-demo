...
static uint32_t  kEstimateDeallocStackSize = 91; //释放栈平均大小，释放栈平均depth为15，ARM64下栈大小为15*5，HYThreadStack大小16Byte
...
@implementation HYZombieDetector
{
...
+ (instancetype)sharedInstance {
    static HYZombieDetector *s_instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        s_instance = [HYZombieDetector new];
    });
    return s_instance;
}
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        self.detectStrategy = HYZombieDetectStrategyCustomObjectOnly;
...
- (void)newDealloc:(__unsafe_unretained id)obj {
    if ([self shouldDetect:[obj class]]) {
        ...
        if (memSize >= [HYZombie zombieInstanceSize]) {//有足够的空间才覆盖
            ...
            memset(p, 0x00, [HYZombie zombieInstanceSize]);
            
            ///把我们自己的类的isa复制过去
            Class c = [HYZombie zombieIsa];
            memcpy(obj, &c, sizeof(void*));
            
            HYZombie* zombie = (HYZombie*)p;
            
            zombie.realClass = origClass;
            
            if (_traceDeallocStack) {
                HYThreadStack *stack = hy_getCurrentStack();
...
- (void)freeZombieObject:(void*)obj {
    HYZombie* zombieObject = (__bridge HYZombie*)obj;
