...
- (instancetype)init {
    ...
    return [self initWithRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@""]] options:0 scale:[UIScreen mainScreen].scale cache:nil cacheKey:nil progress:nil transform:nil completion:nil];
}
            ...
            if (_cache) {
                if (image || (_options & YYWebImageOptionRefreshImageCache)) {
                    ...
                    dispatch_async([YYWebImageOperation _imageQueue], ^{
                        YYImageCacheType cacheType = (_options & YYWebImageOptionIgnoreImageDecoding)?YYImageCacheTypeDisk:YYImageCacheTypeAll;
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[YYImageDecoder alloc] initWithScale:_scale?_scale:[UIScreen mainScreen].scale];
        }
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                CGFloat scale = self->_scale?self->_scale:[UIScreen mainScreen].scale;
                if (allowAnimation) {
                    image = [[YYImage alloc] initWithData:self.data scale:scale];
                    if (shouldDecode) image = [image yy_imageByDecoded];
                    if ([((YYImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    YYImageDecoder *decoder = [YYImageDecoder decoderWithData:self.data scale:scale];
